...
class AngleBranchS2AHead(AngleBranchRetinaHead):
    def __init__(self,
                 *args,
    ...
    def forward_single(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        # bbox_pred[:, [-1], :, :] = self.angle_coder.decode(
        #     angle_pred.permute(0, 2, 3, 1).reshape(-1, self.encode_size)).reshape(
        #     angle_pred.shape[0], angle_pred.shape[2], angle_pred.shape[3], 1).permute(0, 3, 1, 2)
    ...
    def filter_bboxes(self, cls_scores: List[Tensor],
        ...
        for lvl in range(num_levels):
            ...
            for img_id in range(num_imgs):
                ...
                # assert (bbox_pred_i[:,4] == angle_pred_i).all()
                bbox_pred_i[:,-1] = angle_pred_i
    ...
    def loss_by_feat_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        if self.loss_bbox_type == 'normal':
            # loss_bbox=(bbox_pred - bbox_targets).sum()
...
@MODELS.register_module()
class AngleBranchS2ARefineHead(AngleBranchRetinaHead):
    ...
    def forward_single(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        if self.use_normalized_angle_feat:
            angle_pred = angle_pred.sigmoid() * 2 - 1
        # bbox_pred[:,[-1],:,:]=self.angle_coder.decode(angle_pred.permute(0, 2, 3, 1).reshape(-1, self.encode_size)).reshape(
        #     angle_pred.shape[0], angle_pred.shape[2], angle_pred.shape[3], 1).permute(0, 3, 1, 2)
        return cls_score, bbox_pred, angle_pred
    ...
    def loss_by_feat(self,
        ...
        return super(AngleBranchS2ARefineHead, self).loss_by_feat(
    ...
    def loss_by_feat_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        # 新加的
        # Shield angle in reg. branch
        if self.shield_reg_angle:# and self.loss_bbox_type != 'kfiou'
            pass
            # bbox_weights[:, -1] = 0.
        # 新加的结束
